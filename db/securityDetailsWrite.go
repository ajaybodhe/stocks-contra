...
func WriteSecurityDetails(db util.DB, mcss map[string]*coreStructures.MoneyControlSecurityStructure) error {
	securityDetailsWriteSql := "insert into MoneyControlSecurityDetails (symbol, eps, pe, industry_pe, market_cap, book_value, dividend, pb, pc, face_value, div_yield, promoter_holding, fii_holding, dii_holding, other_holding) values "
	firstTime := true
	for k, v := range mcss {
		if firstTime {
