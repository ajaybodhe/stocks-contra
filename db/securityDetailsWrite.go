...
func WriteSecurityDetails(db util.DB, mcss map[string]*coreStructures.MoneyControlSecurityStructure) error {
	...
	for k, v := range mcss {
		if firstTime {
			securityDetailsWriteSql = securityDetailsWriteSql + fmt.Sprintf("(\"%v\", %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v)", k, v.EPS, v.PE, v.IndustryPE, v.MarketCap, v.BookValue, v.Dividend, v.PB, v.PC, v.FaceValue, v.DivYield, v.PromoterHolding, v.DIIHolding, v.FIIHolding, v.OtherHolding)
			firstTime = false
		} else {
			securityDetailsWriteSql = securityDetailsWriteSql + fmt.Sprintf(",(\"%v\", %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v, %v)", k, v.EPS, v.PE, v.IndustryPE, v.MarketCap, v.BookValue, v.Dividend, v.PB, v.PC, v.FaceValue, v.DivYield, v.PromoterHolding, v.DIIHolding, v.FIIHolding, v.OtherHolding)
		}
